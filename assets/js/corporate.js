...
function initCorporatePage(){
  ...
  let lastRequestedHighlight = null;
  let hasHighlightRequest = false;

  function loadHeatmapModule() {
    if (heatmapModule) {
      return Promise.resolve(heatmapModule);
    }
    if (!heatmapModulePromise) {
      heatmapModulePromise = import('./modules/corporate-heatmap.js')
        .then(module => {
          heatmapModule = module;
          heatmapModulePromise = null;
          if (lastHeatmapPayload) {
            applyHeatmapModule(module, lastHeatmapPayload);
          }
          return module;
        })
        .catch(err => {
  ...
  function applyHeatmapModule(module, payload) {
    const result = module.renderCorporateHeatmap({
      grid: payload.grid,
      heatmap: payload.heatmap,
      state,
      t,
      onSelect: handleHeatmapSelection
    });
    state.heatmapCells = result.cells;
    state.updateHeatmapHighlight = result.updateHighlight;
    if (hasHighlightRequest) {
      result.updateHighlight(lastRequestedHighlight);
    } else if (state.selectedColumn?.index != null) {
      result.updateHighlight(state.selectedColumn.index);
    }
  }
  ...
  function renderHeatmap(heatmap){
    ...
    if (!heatmap || !Array.isArray(heatmap.rows) || !Array.isArray(heatmap.cols)) {
      ...
      state.heatmapColumns = Array.isArray(heatmap?.cols) ? heatmap.cols : [];
      state.heatmapDates = Array.isArray(heatmap?.dates) ? heatmap.dates : [];
      if (heatmapModule?.renderEmptyHeatmap) {
        heatmapModule.renderEmptyHeatmap(grid, t);
        if (typeof heatmapModule.clearHeatmapState === 'function') {
          heatmapModule.clearHeatmapState(state);
        }
      } else {
    ...
    const payload = { grid, heatmap };
    lastHeatmapPayload = payload;

    if (heatmapModule) {
      applyHeatmapModule(heatmapModule, payload);
      return;
    }

    loadHeatmapModule().catch(err => {
      devError('Heatmap chunk load failed', err);
    });
  }
  ...
  function updateHeatmapHighlight(index){
    lastRequestedHighlight = index;
    hasHighlightRequest = true;
    if (typeof state.updateHeatmapHighlight === 'function') {
      state.updateHeatmapHighlight(index);
    }
  }
